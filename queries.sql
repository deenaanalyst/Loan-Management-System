...
SELECT *,if(applicantincome > 15000,'Grade A',if(applicantincome > 9000,"Grade B",
if(applicantincome > 5000,'Middle Class','Low Class'))) as Grades
From customer_income;

CREATE TABLE customer_income_critiria(SELECT *,if(applicantincome > 15000,'Grade A',if(applicantincome > 9000,"Grade B",
if(applicantincome > 5000,'Middle Class','Low Class'))) as Grades
FROM customer_income);

SELECT * FROM customer_income_critiria;
DESCRIBE customer_income_critiria;

CREATE TABLE customer_income_status SELECT *,case
when Applicantincome<5000
then case when Property_area='Rural'
then 3 when Property_area='Semirural'
then 3.5 when Property_area='Urban'
then 5 when Property_area='Semiurban'
then 2.5 else 7
end else 7
...
create table loan_status(loan_id text(20),customer_id text(20),loan_amount text(20),
loan_amount_term int,cibil_score int);

create table loan_update_details(loan_id text(20),loan_amount text(20),cibil_score int,
...
describe loan_update_details;

create table customer_interest_analaysis 
(select c.loan_id,c.customer_id,c.applicantincome,c.property_area,
c.grades,concat(c.interest_per,"%") as Interest_per,l.loan_amount,l.cibil_score,l.cibil_score_status,
...
from customer_income_status c inner join loan_update_details l on
c.loan_id=l.loan_id);

select * from customer_interest_analaysis;
drop table customer_interest_analaysis;
commit;
...
select c.* from customer_det c
left join customer_interest_analaysis i on
